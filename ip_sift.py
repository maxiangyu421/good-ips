...
# 校准数据 probe run 34988528820 (N=800, GitHub runner, 同批候选):
#   半开握手过 178 (22.2%) / 全量取回过 42 (5.2%)
#   两者都过 15, 仅半开过 163 (假活 91.6%), 仅全量过 27 (反向误杀)
#   幸存者 ip-api 住宅浓度: 臂B 39% vs 臂A 11% (3.5x)
# ⚠️ 这道闸不是「单调收窄」而是「换一批代理」: 必须同时看开关前后的
#    【送盾数 + 过盾率】, 只看通过率下降会误判成退化。
...
def full_fetch_gate(alive_raw):
    """在「握手存活」与「ip-api 质量筛」之间插入全量取回闸门。
    S1_FULL_GATE=0 时原样放行(默认, 可回滚)。返回 (幸存 [(ms,p)], 统计 dict)。"""
    if not GATE_ON:
    ...
    keep.sort()
    return keep, {"enabled": True, "in": len(cands), "out": len(keep),
...
if __name__ == "__main__":
    ...
    if GATE_ON:
        _keep, gate_stat = full_fetch_gate(alive_raw)
        ...
        # 全量失败 = 代理自身不行 -> 记 s1_dead; 但不误伤已在 good 的
        _keep_hosts = {p.split(":")[0] for _, p in _keep}
        for p in cand:
            if results[p][0] and p.split(":")[0] not in _keep_hosts:
                s1_fresh.add(p.split(":")[0])
        alive_raw = _keep
